...
export const Icon: React.FC<IconProps> = (({
  ...
  return (
    <>
      {
        cloneElement(children as unknown as ElementWithClassName, icon({size, type, className}))
      }
    </>
  )
})
  ...
  >((
    {
      ...
      const iconOnly = React.Children.toArray(children).some(child => 
          React.isValidElement(child) && child.type === Icon && (child as React.ReactElement<IconProps>).props.type === 'only'
      );
